...
template <> class CryptoContextImpl<DCRTPoly> {
  ...
  public:
	...
	Ciphertext<DCRTPoly> ProcessArrayPrecomputations(const Ciphertext<DCRTPoly>& c, const std::vector<std::pair<int, int>>& mask_roll_pairs, int mask_size, int rep, int bits) {
